...
import 'package:delicias_online/views/Recheio/EditarRecheioPage.dart';
...
class AddRecheioPage extends StatefulWidget {
  const AddRecheioPage({Key? key}) : super(key: key);

  @override
  State<AddRecheioPage> createState() => _AddRecheioPageState();
}

class _AddRecheioPageState extends State<AddRecheioPage> {
  final _formKey = GlobalKey<FormState>();
  final _descricaoController = TextEditingController();
  final _valorController = TextEditingController();
  bool _isLoading = false;
  List<RecheioModel> _recheios = [];
  ...
  Future<void> _carregarRecheios() async {
    ...
    try {
      ...
      if (confeitariaId != null) {
        final recheios = await RecheioService.getRecheiosPorConfeitaria(int.parse(confeitariaId));
        setState(() => _recheios = recheios);
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Erro ao carregar recheios: ${e.toString()}'),
  ...
  Future<void> _salvarRecheio() async {
    ...
    try {
      ...
      final response = await RecheioService.cadastrarRecheio(
      ...
      if (response['status'] == 'success') {
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const Text('Recheio cadastrado com sucesso!'),
              ],
            ),
            actions: [
              TextButton(
        ...
        await _carregarRecheios();
      } else {
        // Mensagem de erro detalhada
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response['message'] ?? 'Erro ao cadastrar recheio'),
  ...
  Future<void> _excluirRecheio(int id) async {
    ...
    builder: (context) => AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          const Text('Tem certeza que deseja excluir este recheio?'),
  ...
  try {
    ...
    if (response.statusCode == 200) {
        ...
        builder: (context) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Icon(Icons.check_circle, color: Colors.green, size: 50),
              const SizedBox(height: 20),
              Text(data['message'] ?? 'Recheio excluído com sucesso!'),
            ],
          ),
          actions: [
            TextButton(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Erro: ${data['message'] ?? 'Falha ao excluir recheio'}'),
          backgroundColor: Colors.red,
          duration: const Duration(seconds: 3),
          behavior: SnackBarBehavior.floating,
          action: SnackBarAction(
  ...
  void _navegarParaEdicao(RecheioModel recheio) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => EditarRecheioPage(
          recheio: recheio,
          onRecheioAtualizado: _carregarRecheios,
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Gerenciar Recheios'),
        elevation: 0,
                      ...
                      children: [
                        ...
                        _isLoading
                            ...
                            : FilledButton(
                                ...
                                child: const Text('Salvar Recheio'),
                              ),
                      ],
                    ),
                  ),
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () => _navegarParaEdicao(recheio),
                                    ),
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        showDialog(
                                          ...
                                          builder: (context) => AlertDialog(
                                            title: const Text('Confirmar Exclusão'),
                                            content: Text('Excluir "${recheio.descricao}"?'),
                                            actions: [
                                              TextButton(
                                              ...
                                              TextButton(
                                                onPressed: () async {
                                                  Navigator.pop(context);
                                                  await _excluirRecheio(recheio.id!);
                                                },
                                                child: Text(
                                                  'Excluir',
