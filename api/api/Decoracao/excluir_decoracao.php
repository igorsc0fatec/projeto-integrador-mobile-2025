...
try {
    ...
    // Verifica se a decoração existe
    $stmtCheck = $pdo->prepare("SELECT id_decoracao FROM tb_decoracao WHERE id_decoracao = ?");
    $stmtCheck->execute([$id]);
    
    if ($stmtCheck->rowCount() === 0) {
        throw new Exception("Decoração não encontrada", 404);
    }

    // Exclui a decoração
    $stmt = $pdo->prepare("DELETE FROM tb_decoracao WHERE id_decoracao = ?");
    ...
    if ($success) {
        ...
        echo json_encode([
            'status' => 'success', 
            'message' => 'Decoração excluída com sucesso',
            'data' => ['id' => $id]
        ]);
    } else {
        throw new Exception("Falha ao excluir decoração", 500);
    }
