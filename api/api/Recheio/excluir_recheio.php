...
try {
    ...
    // Verifica se o recheio existe
    $stmtCheck = $pdo->prepare("SELECT id_recheio FROM tb_recheio WHERE id_recheio = ?");
    $stmtCheck->execute([$id]);

    if ($stmtCheck->rowCount() === 0) {
        throw new Exception("Recheio não encontrado", 404);
    }

    // Exclui o recheio
    $stmt = $pdo->prepare("DELETE FROM tb_recheio WHERE id_recheio = ?");
    ...
    if ($success) {
        ...
        echo json_encode([
            'status' => 'success',
            'message' => 'Recheio excluído com sucesso',
            'data' => ['id' => $id]
        ]);
    } else {
        throw new Exception("Falha ao excluir recheio", 500);
    }
