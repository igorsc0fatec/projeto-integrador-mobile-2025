...
function handleGetRequest() {
    ...
    try {
        $stmt = $conn->prepare("SELECT * FROM tb_cobertura WHERE id_confeitaria = ?");
        ...
        while ($row = $result->fetch_assoc()) {
            $coberturas[] = [
                ...
                'desc_cobertura' => $row['desc_cobertura'],
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'Erro ao listar coberturas: ' . $e->getMessage()
        ]);
    }
}
...
function handlePostRequest() {
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO tb_cobertura (desc_cobertura, valor_por_peso, id_confeitaria) VALUES (?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode([
                'status' => 'success',
                'message' => 'Cobertura cadastrada com sucesso',
                'data' => [
                    'id_cobertura' => $id,
                    'desc_cobertura' => $data->descricao,
            ...
            echo json_encode([
                'status' => 'error',
                'message' => 'Erro ao cadastrar cobertura'
            ]);
        }
    } catch (Exception $e) {
        echo json_encode([
