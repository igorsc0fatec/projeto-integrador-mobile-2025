...
try {
    ...
    $stmtCheck = $pdo->prepare("SELECT id_cobertura FROM tb_cobertura WHERE id_cobertura = ?");
    $stmtCheck->execute([$id]);
    
    if ($stmtCheck->rowCount() === 0) {
        throw new Exception("Cobertura não encontrada", 404);
    }

    // Exclui a cobertura
    $stmt = $pdo->prepare("DELETE FROM tb_cobertura WHERE id_cobertura = ?");
    ...
    if ($success) {
        ...
        throw new Exception("Falha ao excluir cobertura", 500);
    }
