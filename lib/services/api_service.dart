...
import 'package:delicias_online/models/confeitaria.dart';
...
class ApiService {
  ...
  // Obter dados da confeitaria por ID do usuário
    static Future<Confeitaria> getConfeitaria(int userId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/get_confeitaria.php?userId=$userId'),
    );

    if (response.statusCode == 200) {
      return Confeitaria.fromJson(jsonDecode(response.body));
    } else {
